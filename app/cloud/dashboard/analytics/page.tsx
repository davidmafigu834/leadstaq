...
function ShareRow({ project, origin }: { project: Project; origin: string }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 0", borderBottom: "0.5px solid rgba(0,0,0,0.06)" }}>
      {/* Folder icon */}
      <div style={{ width: 40, height: 40, borderRadius: 12, background: "#F0F0F4", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontSize: 15, fontWeight: 600, color: "#111111", margin: "0 0 2px", fontFamily: F, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{project.title}</p>
        <p style={{ fontSize: 12, color: "#9CA3AF", margin: 0, fontFamily: F, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{shareUrl.replace("https://", "")}</p>
      </div>
      {/* Meta */}
      <div style={{ textAlign: "right", flexShrink: 0 }}>
        <p style={{ fontSize: 13, fontWeight: 600, color: "#374151", margin: "0 0 2px", fontFamily: F }}>{photoCount} photo{photoCount !== 1 ? "s" : ""}</p>
        <p style={{ fontSize: 11, color: "#9CA3AF", margin: 0, fontFamily: F }}>{formatDate(project.updated_at)}</p>
      </div>
      {/* Copy button */}
      <button
        onClick={copy}
        style={{ height: 34, padding: "0 14px", background: copied ? "#D4FF4F" : "#F0F0F4", color: copied ? "#111111" : "#374151", fontSize: 13, fontWeight: 600, borderRadius: 9, border: "none", cursor: "pointer", flexShrink: 0, fontFamily: F, transition: "background 0.2s" }}
      >
        {copied ? "Copied!" : "Copy"}
      </button>
    </div>
  ...
  if (status === "loading" || loading) {
    return (
      <div style={{ minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ width: 28, height: 28, borderRadius: "50%", border: "3px solid #D4FF4F", borderTopColor: "transparent", animation: "spin 0.7s linear infinite" }} />
      </div>
    );
  }
    ...
    <div style={{ minHeight: "100vh", background: "#F7F7F8", fontFamily: F, paddingBottom: 100 }}>
      ...
      <div style={{ padding: "16px 20px 0" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          {([
            ...
            <div key={s.label} style={{ background: "#FFFFFF", borderRadius: 16, border: "0.5px solid rgba(0,0,0,0.07)", padding: "14px 12px", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
              ...
              <p style={{ fontFamily: S, fontSize: 24, color: "#111111", margin: "0 0 2px", lineHeight: 1 }}>{s.value}</p>
              <p style={{ fontSize: 11, color: "#9CA3AF", margin: 0, fontFamily: F }}>{s.label}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: "20px 20px 0" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.09em", textTransform: "uppercase", color: "#4B5563", margin: 0, fontFamily: F }}>Shared project links</p>
          {projects.length > 0 && (
            <span style={{ fontSize: 12, color: "#9CA3AF", fontFamily: F }}>{projects.length} total</span>
          )}
        </div>
        <div style={{ background: "#FFFFFF", borderRadius: 20, border: "0.5px solid rgba(0,0,0,0.07)", padding: "0 16px", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
          {projects.length === 0 ? (
            <div style={{ padding: "32px 0", textAlign: "center" }}>
              <p style={{ fontSize: 15, color: "#9CA3AF", margin: 0, fontFamily: F }}>No projects yet</p>
            </div>
          ) : (
            projects.map((p, i) => (
      ...
      {categories.length > 0 && (
        <div style={{ padding: "20px 20px 0" }}>
          <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.09em", textTransform: "uppercase", color: "#4B5563", margin: "0 0 12px", fontFamily: F }}>Projects by category</p>
          <div style={{ background: "#FFFFFF", borderRadius: 20, border: "0.5px solid rgba(0,0,0,0.07)", padding: "16px", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
                ...
                <div key={cat} style={{ marginBottom: 14 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <span style={{ fontSize: 14, fontWeight: 600, color: "#374151", fontFamily: F }}>{cat}</span>
                    </div>
                    <span style={{ fontSize: 13, color: "#6B7280", fontFamily: F }}>{count} project{count !== 1 ? "s" : ""} · {pct}%</span>
                  </div>
      ...
      {projects.length > 0 && (
        <div style={{ padding: "20px 20px 0" }}>
          <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.09em", textTransform: "uppercase", color: "#4B5563", margin: "0 0 12px", fontFamily: F }}>Top projects by photos</p>
          <div style={{ background: "#FFFFFF", borderRadius: 20, border: "0.5px solid rgba(0,0,0,0.07)", padding: "16px", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
            {[...projects]
              ...
              .map((p, i) => {
                ...
                return (
                  <div key={p.id} onClick={() => router.push(`/cloud/dashboard/projects/${p.id}`)}
                    ...
                    <span style={{ fontFamily: S, fontSize: 18, color: "#D1D5DB", width: 20, textAlign: "right", flexShrink: 0 }}>{i + 1}</span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                        <span style={{ fontSize: 14, fontWeight: 600, color: "#111111", fontFamily: F, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.title}</span>
                        <span style={{ fontSize: 13, color: "#6B7280", fontFamily: F, flexShrink: 0, marginLeft: 8 }}>{count}</span>
                      </div>
