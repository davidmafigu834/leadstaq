    ...
    <div className="max-w-[1100px] mx-auto px-6 py-5">
      {projects.length === 0 ? (
        ...
        <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
          ...
          <h2 className="mb-2 text-[24px] font-semibold tracking-tight text-white">Add your first project</h2>
          <p className="mb-6 max-w-xs text-[15px] text-[#555]">
            Start by creating a project for a job you&apos;re working on. Then upload photos
            straight from your phone.
          </p>
          <button
            onClick={() => setShowNew(true)}
            className="flex items-center gap-2 rounded-md bg-[#D4FF4F] px-4 py-2 text-[14px] font-semibold text-black hover:bg-[#c8f244] transition-colors"
          >
        ...
        <>
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-3">
              <p className="text-[12px] font-medium text-[#555] uppercase tracking-[0.08em]">01 / RECENT PROJECTS</p>
              <Link
                href="/cloud/dashboard/projects"
                className="flex items-center gap-1 text-[13px] text-[#555] hover:text-[#888] transition-colors"
              >
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {recent.map((p) => (
                ...
                >
                  ...
                  />
                  {p.category && (
                    <span className="absolute top-2.5 left-2.5 flex h-5 items-center px-2 text-[12px] font-medium rounded bg-black/60 text-white/80 border border-white/10 backdrop-blur-sm">
                      {p.category}
                    </span>
                  )}
                  <div className="absolute bottom-2.5 left-3 right-3">
                    <p className="text-[15px] font-semibold text-white leading-tight">{p.title}</p>
              ...
              >
                ...
                <span className="text-[14px] text-[#555]">New project</span>
              </button>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 px-4 py-3 bg-[#111] border border-white/[0.08] rounded-lg hover:border-white/[0.15] transition-colors cursor-pointer">
            ...
            <div className="flex-1">
              <p className="text-[15px] text-[#888] font-medium">Upload photos to an existing project</p>
            </div>
            <Link
              href="/cloud/dashboard/upload"
              onClick={(e) => e.stopPropagation()}
              className="flex items-center gap-1.5 h-7 px-3 bg-transparent border border-white/[0.15] text-[13px] text-[#888] rounded-md hover:border-white/[0.3] hover:text-white transition-colors shrink-0"
            >
              Upload
            </Link>
          </div>
