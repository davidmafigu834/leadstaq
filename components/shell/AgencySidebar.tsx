  ...
  return (
    <Link
      ...
      className={`relative flex h-9 items-center gap-3 rounded-sm py-2 text-[13px] font-medium transition-colors ${
        forceExpanded ? "justify-start px-3" : "justify-center px-2 layout:justify-start layout:px-3"
      } ${
        isActive
          ? "text-[var(--accent)]"
    ...
    >
      ...
      <span className={`min-w-0 flex-1 truncate ${forceExpanded ? "inline" : "hidden layout:inline"}`}>{item.label}</span>
      {item.badge != null && item.badge > 0 ? (
        <span
          className={`rounded-sm bg-[var(--accent)] px-1.5 py-0 font-mono text-[10px] font-medium text-[var(--accent-ink)] ${forceExpanded ? "inline" : "hidden layout:inline"}`}
        >
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col">
      <div className="shrink-0 px-3 pb-4 pt-6 layout:px-5">
        <Link href={homeHref} className={`flex items-center gap-2 ${forceExpanded ? "justify-start" : "justify-center layout:justify-start"}`}>
          <span
            className={`mt-1 h-1.5 w-1.5 shrink-0 bg-[var(--accent)] ${forceExpanded ? "hidden" : "layout:hidden"}`}
            aria-hidden
          />
          <div className={`${forceExpanded ? "block" : "hidden layout:block"} text-left`}>
            {sidebarBrand ? (
        ...
        <div className={`mt-5 h-px bg-[var(--surface-sidebar-border)] ${forceExpanded ? "block" : "hidden layout:block"}`} />
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden overscroll-contain px-1 pb-2 [scrollbar-width:thin] [scrollbar-color:rgba(255,255,255,0.2)_transparent]">
        <nav className="flex flex-col gap-0.5 px-2 layout:px-3">
          {primaryNav.map((item) => (
            <NavRow key={item.href} item={item} navActive={navActive} forceExpanded={forceExpanded} />
          ))}
        </nav>

        <div
          className={`mx-2 my-2 h-px bg-[var(--surface-sidebar-border)] ${forceExpanded ? "block" : "hidden layout:mx-3 layout:block"}`}
        />
        ...
        {clients && clients.length > 0 ? (
          <div className={`${forceExpanded ? "block" : "hidden layout:block"} px-3 pb-2 pt-2`}>
        ...
        <details className="group relative px-2 pb-2 layout:px-3">
          <summary
            className={`flex cursor-pointer list-none items-center gap-2 rounded-sm py-2 marker:hidden hover:bg-[var(--surface-sidebar-elevated)] [&::-webkit-details-marker]:hidden ${
              forceExpanded ? "justify-start px-2" : "justify-center px-1 layout:justify-start layout:px-2"
            }`}
          >
            <ClientAvatar name={userName} size="sm" />
            <div className={`${forceExpanded ? "block" : "hidden layout:block"} min-w-0 flex-1 text-left`}>
            ...
            <ChevronRight
              className={`h-4 w-4 shrink-0 text-[var(--text-on-dark-dim)] transition group-open:rotate-90 ${forceExpanded ? "block" : "hidden layout:block"}`}
            />
          </summary>
